...
def get_pointnet_pred(points, model, batch_size):
    output_features = []
    ...
    for batch_index in range(batch_cnt):
        ...
        output_features.append(batch_feature[0].exp().cpu().detach().numpy())
...
if __name__ == '__main__':
    ...
    if os.path.exists(f'{args.pred_folder}/pis.csv'):
        print("pis.csv already exists, skipping")
        exit()
    ...
    result_pred_prediction = get_pointnet_pred(result_pred_points, pointnet, args.batch_size)
    ...
    df.to_csv(f'{args.pred_folder}/pis.csv', index=False)
